...
**超卖策略**：
- **CPU 超卖比**: 10:1（request: 0.1C, limit: 1C）
- **内存超卖比**: 5:1（request: 400MB, limit: 2GB）
...
4. **成熟度高**: TKE 集群已被小红书、美团等公司验证（10 万+ Pod）
...
| **成熟度** | ✅ 已验证（小红书 10 万+ Pod） | ⚠️ 百万级 Pod 待验证 |
...
1. **小红书**: 10 万+ Pod TKE 集群，高超卖（CPU 10:1）
2. **美团**: TKE Serverless + CBS，强调 5 秒启动体验
3. **微信内部（David 团队）**: Host Path 挂载 + 超卖 10 倍，锁定节点调度
4. **Kimi**: TKE Serverless，虚拟机隔离

### 经验总结

- **启动速度**: 15 秒内可接受，10 秒为较好体验
- **超卖比**: CPU 10:1 比较激进但可行，内存 5:1 较保守
